...
# ==============================
# Runtime / Bundle Root Handling
# ==============================

def get_bundle_root() -> Path:
    """
    Where PyInstaller onefile extracts embedded resources (sys._MEIPASS).
    Not where bin/ or output/ live.
    """
...
def get_runtime_root() -> Path:
    """
    Where the EXE lives (or project root when running from source).
    This is where we expect bin/, output/, backups/, README/LICENSE, etc.
    """
    if getattr(sys, "frozen", False):
        exe_dir = Path(sys.executable).resolve().parent
        # Some onedir builds store resources in _internal
...
# Back-compat names used throughout the script
...
def cleanup_dic_sidecar_wavs(dic_path: Path, log: Logger) -> int:
    """
    Some vgmstream/test.exe builds create sidecar WAVs next to the .dic, e.g:
        NESSamples.dic#1.wav
    These are safe to delete (they are not user music, not backups, not output).
    Returns number deleted.
    """
    try:
        folder = dic_path.parent
        base = dic_path.name
        patterns = [
            f"{base}#*.wav",
            f"{base}#*.WAV",
        ...
        to_delete: List[Path] = []
        for pat in patterns:
            to_delete.extend([p for p in folder.glob(pat) if p.is_file()])
        # de-dupe
        seen = set()
        uniq: List[Path] = []
        for p in to_delete:
        ...
        if deleted:
            log.info(f"Auto-cleaned sidecar WAVs: {deleted} in {folder}")
        return deleted
    except Exception as e:
        log.warn(f"Sidecar WAV cleanup failed: {e}")
        return 0
