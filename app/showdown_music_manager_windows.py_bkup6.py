...
def _collect_about_files() -> List[Path]:
    """Collect license/readme/notice files from runtime root (packaged) and project root (source)."""
    paths: List[Path] = []
    ...
    # If packaged and APP_ROOT is _internal, also consider the folder containing the EXE
    if APP_ROOT.name.lower() == "_internal":
        roots.append(APP_ROOT.parent)

    # Also consider current working directory (common when launched from release folder)
    ...
    for root in uniq_roots:
        add_matches(root)
        add_matches(root / "bin")
        add_matches(root / "bin" / "ffmpeg")
        add_matches(root / "bin" / "vgmstream-win")
        add_matches(root / "bin" / "vgmstream")
        add_matches(root / "bin" / "tools")
...
def show_about_menu() -> None:
    ...
    while True:
        ...
        console.print(Panel.fit(
            ...
            f"[dim]Root:[/dim] {root}\n[dim]CWD:[/dim] {Path.cwd()}\n[dim]Exe:[/dim] {Path(sys.executable).resolve() if getattr(sys, 'frozen', False) else 'python'}",
            border_style="cyan"
        ))
