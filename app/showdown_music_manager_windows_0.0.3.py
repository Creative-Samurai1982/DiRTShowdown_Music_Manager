...
def resolve_vgmstream_exe() -> Path:
    """Resolve vgmstream CLI in a variety of release layouts.

    We support multiple common folder structures because users often ship either:
      - bin\vgmstream-win\vgmstream-cli.exe
      - bin\vgmstream\vgmstream-cli.exe
      - bin\vgmstream-cli.exe
      - bin\test.exe (legacy wrapper)
    """
    candidates = [
        # Preferred layouts
        r"bin\vgmstream-win\vgmstream-cli.exe",
        r"bin\vgmstream-win\test.exe",
        r"bin\vgmstream\vgmstream-cli.exe",
        r"bin\vgmstream\test.exe",

        # Flat bin layouts
        r"bin\vgmstream-cli.exe",
        r"bin\test.exe",

        # Root-level fallbacks (rare)
        r"vgmstream-cli.exe",
        r"test.exe",
    ...
    # Build a helpful error message with what we actually tried on disk
    tried = []
    base = APP_ROOT
    for c in candidates:
        # Only show absolute attempts for relative paths; bare names are PATH-only attempts.
        if "\\" in c or "/" in c or c.lower().endswith(".exe"):
            tried.append(str((base / c).resolve()))

    raise FileNotFoundError(
        "vgmstream CLI not found.\n"
        f"APP_ROOT: {base}\n"
        "Expected one of these paths to exist (or vgmstream-cli on PATH):\n"
...
def _collect_about_files() -> List[Path]:
    """Collect license/readme/notice files.

    Priority (most important first):
      1) Runtime root (folder where EXE lives / portable release folder)
      2) Runtime parent (covers onedir '_internal' layouts)
      3) Current working directory
      4) Source project root (when running from Python in /app)
      5) Bundle root (PyInstaller onefile _MEIPASS), if present
    """
    paths: List[Path] = []
    ...
    # 1) Runtime (preferred)
    add_matches(APP_ROOT)

    # 2) Parent (if APP_ROOT is _internal, docs usually sit next to the EXE)
    add_matches(APP_ROOT.parent)

    # 3) CWD fallback
    try:
        add_matches(Path.cwd())
    except Exception:
        pass

    # 4) Source project root (app/ layout)
    try:
        script_dir = Path(__file__).resolve().parent
        proj_root = script_dir.parent if script_dir.name.lower() == "app" else script_dir
        add_matches(proj_root)
    except Exception:
        pass

    # 5) Bundle root (onefile extraction), only if present
    meipass = getattr(sys, "_MEIPASS", None)
    if meipass:
        add_matches(Path(meipass).resolve())

    # Known third-party folders (support multiple bin layouts)
    for base in [APP_ROOT, APP_ROOT.parent]:
        add_matches(base / "bin")
        add_matches(base / "bin" / "ffmpeg")
        add_matches(base / "bin" / "ffmpeg" / "bin")
        add_matches(base / "bin" / "vgmstream-win")
        add_matches(base / "bin" / "vgmstream")
...
def show_about_menu() -> None:
    ...
    IMPORTANT: In PyInstaller onefile mode, __file__ points into _MEI... temp.
    We must use APP_ROOT / sys.executable folder for runtime docs.
    """
    files = _collect_about_files()
    root = APP_ROOT
    cwd = Path.cwd()
    exe = Path(sys.executable).resolve() if getattr(sys, "frozen", False) else None
    meipass = getattr(sys, "_MEIPASS", None)
    ...
    while True:
        ...
        info_lines = [
            ...
            "View bundled third-party notices and licence/readme files.",
            f"[dim]Runtime Root:[/dim] {root}",
            f"[dim]CWD:[/dim] {cwd}",
        ]
        if exe:
            info_lines.append(f"[dim]EXE:[/dim] {exe}")
        if meipass:
            info_lines.append(f"[dim]Bundle (_MEIPASS):[/dim] {meipass}")
        ...
        console.print(table)
        console.print()
        console.print("[dim]Enter a number to view, or press Enter to return.[/dim]")
        ans = Prompt.ask("Choice", default="")
        ...
        try:
            ...
            try:
                ...
                console.print(f"[red]Failed to read file:[/red] {p}\n{e}")
...
def advanced_full_scan_build_profiles(
    ...
    def worker(dic_path: Path) -> Tuple[str, Optional[Profile], str]:
        ...
        try:
            ...
            cleanup_dic_sidecar_wavs(dic_path, log)
...
def cleanup_all_dic_sidecar_wavs(game_root: Path, log: Logger, console: Optional[Console] = None) -> int:
    """
    Delete vgmstream sidecar WAVs created next to DICs:
      <something>.dic#123.wav

    Safe scope: only under game_root/audio/dictionaries.
    """
    dic_root = game_root / "audio" / "dictionaries"
    if not dic_root.exists():
        return 0

    patterns = ("*.dic#*.wav", "*.dic#*.WAV")
    deleted = 0

    for pat in patterns:
        for p in dic_root.rglob(pat):
            try:
                p.unlink()
                deleted += 1
            except Exception as e:
                log.warn(f"Failed to delete sidecar WAV: {p} :: {e}")

    if deleted and console:
        console.print(f"[green]Cleaned {deleted} DIC sidecar WAV(s).[/green]")
    if deleted:
        log.info(f"Cleaned {deleted} DIC sidecar WAV(s) under {dic_root}")
    return deleted



def cleanup_dic_sidecar_wavs(dic_path: Path, log: Logger) -> int:
    """
    Remove vgmstream sidecar WAVs that appear next to DIC files, e.g.:
      NESSamples.dic#1.wav
    We only delete sidecars for the given DIC (safe; won't touch your music library WAVs).
    Returns number of files deleted.
    """
    dic_dir = dic_path.parent
    stem = dic_path.name  # includes .dic
    patterns = [f"{stem}#*.wav", f"{stem}#*.WAV"]
    deleted = 0
    for pat in patterns:
        for wav in dic_dir.glob(pat):
            if wav.is_file():
                try:
                    wav.unlink()
                    deleted += 1
                except Exception as e:
                    log.warn(f"Could not delete WAV sidecar: {wav} :: {e}")
    if deleted:
        log.info(f"Deleted {deleted} DIC sidecar WAV(s) for {dic_path.name}")
    return deleted
...
def main() -> None:
    ...
    while True:
        try:
            ...
            cleanup_all_dic_sidecar_wavs(game_root, log, console)
        except Exception as e:
            console.print(f"[red]{e}[/red]")
            log.error(str(e))
            return
    ...
    while True:
        ...
        if ckey in cache and cache[ckey].get("fp") == fp:
            ...
            cleanup_dic_sidecar_wavs(dic, log)
            cache[ckey] = {
                "fp": fp,
